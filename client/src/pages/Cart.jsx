  ...
  return (
    <div className="grid grid-cols-1 md:w-2/4 w-full max-md:p-2 mx-auto my-4 gap-3">
      ...
      {products && products.map((item)=>{
       return<section className="py-2 flex justify-between items-center border-2 px-3 ">
        <div className="flex max-md:flex-col gap-2">
            <img className="w-[6rem] object-contain rounded-sm border shadow-sm" src={item.imageUrls[0]} alt="" />
            <p>{item.title}</p>
        </div>            
        <div className="my-3">
         <button  className="p-1 border-2  w-10 hover:bg-[--primary] hover:text-white font-bold mr-2 shadow-sm">+</button>
         <samp>{quantity}</samp>             
         <button className="p-1 border-2  w-10 hover:bg-[--primary] hover:text-white font-bold m-2 shadow-sm">-</button>
        </div>
        <div className="flex flex-col items-center">
            <p className="font-semibold my-2">Tk {item.regularPrice}</p>
     ...
     <span className="flex justify-between items-center border-b-2 py-2">
        ...
        <p className="font-bold">Tk {total} </p>
     </span>
     <span className="flex justify-between items-center border-b-2 py-2">
